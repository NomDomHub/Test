	...
	task.delay(#text * 0.1 + 2, function()
		for i = #letterLabels, 1, -1 do
			task.delay((#letterLabels - i) * 0.1, function()
				tween(letterLabels[i], TweenInfo.new(1, Enum.EasingStyle.Sine, Enum.EasingDirection.In), {
					Position = letterLabels[i].Position + UDim2.new(0, 0, 0.5, 0),
					TextTransparency = 1
				}):Play()
			end)
		end
...
-- Biến cờ để đánh dấu đã gọi CreateLoading thủ công
local manualCall = false

function LoadingModule:CreateLoading(options)
	options = options or {}
	manualCall = true
	showLoading(options.Title)
end

-- Tự động chạy khi require/loadstring nếu không gọi thủ công CreateLoading
task.spawn(function()
	task.wait(0.1) -- đợi xem có gọi CreateLoading không
	if not manualCall then
		showLoading()
	end
end)
